...
export default Ember.Controller.extend({
    ...
    dates: Ember.computed('model.data.[]', 'activeKeys.[]', function() {
        return ['x'].concat(this.get('model.data').map((datapoint) => {
    ...
    data: Ember.computed('storedMetrics.[]', 'storedMetrics', function() {
        ...
        this.get('storedMetrics').forEach((key) => {
            ...
            let object =  [key].concat(this.get('model.data').map((datapoint) => {
    ...
    projects: Ember.computed('model.data.[]', function() {
        return [
            { 'label': 'UUX', value: 'Q2.UUX' }
        ];
    }),
    subType: Ember.computed('model.data.[]', function() {
        return [
    ...
    metrics: Ember.computed('model.data.[]', function() {
        return [
